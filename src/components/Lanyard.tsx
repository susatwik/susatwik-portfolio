...
function Band({
  ...
  useFrame((state, delta) => {
    ...
    const invalidVector = translations.some(
      (v) =>
        !v ||
        !Number.isFinite(v.x) ||
        !Number.isFinite(v.y) ||
    ...
    [j1, j2].forEach((ref) => {
      ...
      if (
        !Number.isFinite(lerped.x) ||
        !Number.isFinite(lerped.y) ||
        !Number.isFinite(lerped.z)
      ) {
        lerped.set(trans.x, trans.y, trans.z);
      }
      ...
      const distance = lerped.distanceTo(target);
    ...
    if (
      ...
      try {
        ...
        console.warn(
          'Bounding sphere computation skipped:',
          err
        );
      }
    }
    ...
    // Animate texture
    if (strapTexture) {
      strapTexture.offset.x -= delta * 0.1;
    }
  });
      ...
      <group position={[0, 4, 0]}>
        ...
        >
          ...
          <group
            ...
            onPointerUp={(e) => {
              (e.target as any).releasePointerCapture(
                e.pointerId
              );
            ...
            onPointerDown={(e) => {
              (e.target as any).setPointerCapture(
                e.pointerId
              );
